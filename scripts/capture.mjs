...
const failures = [];
const inventory = [];
const pages = new Set(['/', '/home','/camps','/dfp','/camp-staff-and-volunteers','/camp-sponsors','/camp-subsidies','/camp-faq','/mission-en','/history','/staff','/board-of-directors','/donate','/pray-for-us','/planned-giving','/banquet','/financials','/policies','/testimonials','/newsletters','/facility-rental','/contact-us','/careers']);
...
await writeFile('migration/reports/capture.json',JSON.stringify({capturedAt:new Date().toISOString(),routes:[...pages],pageCount:raw.length,assetCount:assets.size,failures,pages:inventory},null,2));
...
if(failures.some(f=>f.kind==='page'))process.exitCode=1;
